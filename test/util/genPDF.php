...
function generatePDFConfirm($resultSet,$outputType,$resendType){
	...
	*/
	...
	/**** get custom fields *****/
	if($resultSet[7] == "8") {
		$queryCustom = "SELECT * FROM tblB2TCustomFields WHERE fkTransID = ".$resultSet[0];
	...
	$pdf->Cell(0,4, "Fax:                       ",0,1,"R");   /* was: 713-572-4224  */
	$pdf->Ln();
	...
	*/
	...
	$pdf->SetFont("Arial","",11);
	$pdf->Cell(0,5,$productGroup,0,1);
	
	//$pdf->Cell(0,5,"Show product: ".$confirm->showProduct()." - ".$sellerInfo[1],0,1);
	
	if($confirm->showProduct() == 1 || $templateOptions[0] == 3 || $resultSet[8] == 4 || $resultSet[8] ==2 ) { /* show for Heat Rate trades */
		$pdf->SetFont("Arial","B",11);
		$pdf->Cell($cellWidth,5,"Product: ",0,0);
		$pdf->SetFont("Arial","",11);
		$pdf->Cell(0,5,getProduct($resultSet[8]),0,1);
	}
	else if($resultSet[7] == "8") { //crude oil custom field output
		$pdf->SetFont("Arial","B",11);
		$pdf->Cell($cellWidth,5,"Product: ",0,0);
		$pdf->SetFont("Arial","",11);
	...
	$pdf->SetFont("Arial","",11);		
	$pdf->Cell(0,5,$confirm->locationConfirmLanguage(getLocationList($resultSet[11]),getProductType($resultSet[9])),0,1);
	...
	if(($templateOptions[3] == "1" && ($confirm->showDelivery() != 0 || $confirm->showDelivery == "")) || $confirm->showDelivery() == 1 ){
		if($resultSet[9] != 2 && $resultSet[9] != 3) { //hide for all Financial deals
			...
			$pdf->SetFont("Arial","",11);
			if($resultSet[7] == "8")
				$pdf->Cell(0,5,$customFields[3],0,1);
			else
	...
	$pdf->SetFont("Arial","",11);
	$pdf->Cell(0,5,$resultSet[26]." ".$tradeUnits,0,1);
