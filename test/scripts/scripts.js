...
function launchTrade(templateID) {
	newWindow("trade/tradeInput.php?tradeType="+templateID+"&height=570",615,900,"no");
}
...
function launchLocation(locationID) {
	...
	if(isIE())
		height += 40;
	newWindow("trade/locationDetail.php?locationID="+locationID,height,640,"no");
}

function launchBroker(brokerID) {
	newWindow("trade/brokerDetail.php?brokerID="+brokerID,335,490,"no");
}
...
function launchDetail(templateID,tradeID) {
	...
	newWindow("trade/tradeInput.php?tradeType="+templateID+"&tradeID="+tradeID+"&height=570",615,900,"no");
}
...
function invoiceDetail(clientID,startDate,endDate) {
	...
	if(endDate == undefined)
		endDate="";
	if(clientID != "")
		newWindow("trade/invoiceDetail.php?companyID="+clientID+"&startDate="+startDate+"&endDate="+endDate+"&height=570",615,900,"no");	
...
function setSame(checkRef) {
	//set client fields the same
	if(checkRef.checked == true){
		$("#confirmClientName").val( $("#invoiceName").val() );
		$("#confirmEmail").val( $("#email").val() );
		$("#confirmStreet1").val( $("#street1").val() );
		$("#confirmStreet2").val( $("#street2").val() );
		$("#confirmCity").val( $("#city").val() );
		$("#confirmState").val( $("#state").val() );
		$("#confirmZip").val( $("#zip").val() );
		$("#confirmPhone").val( $("#phone").val() );
		$("#confirmFax").val( $("#fax").val() );
		$("#confirmCountry").val( $("#country").val() );
	}
	else {
