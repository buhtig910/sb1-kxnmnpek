...
function login($username,$password) {
	...
	if($username != "" && $password != ""){
		$query = "SELECT pkBrokerID,fldUserType FROM tblB2TBroker WHERE fldUsername=\"$username\" AND fldPassword=\"".md5($password)."\" AND fldActive=1";
		//echo($query);
		$result = mysqli_query($_SESSION['db'],$query);
		...
		if($count == 1){
			//echo("<p>logged in</p>");
			list($broker,$userType) = mysqli_fetch_row($result);
...
function populateOffsetList($offsetID) {
	...
	if($locationCount > 0){	
		...
		while($resultSet = mysqli_fetch_row($resultLocation)){
			...
			*/
			...
			if($locationID == $resultSet[0])
				$selected = "selected=\"selected\"";
			else if($locationID == "" && $resultSet[1] == "NYMEX Natural Gas (LD1)")
				$selected = "selected=\"selected\"";
...
function getUserTypeList($brokerID) {
...
function populateExistingCompanies($companyID) {
	...
	$query = "SELECT co.pkCompanyID,co.fldCompanyName,count(cl.pkClientID) FROM tblB2TCompany as co LEFT JOIN tblB2TClient as cl on cl.fkCompanyID=co.pkCompanyID WHERE cl.fldActive=1 GROUP BY pkCompanyID ORDER BY co.fldCompanyName ASC";
...
function translateProduct($product) {
	...
	if($productGroup == "US Electricity")
		return "Power";
	else
		return "Gas";
