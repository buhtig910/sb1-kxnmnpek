	...
	********************************/
	...
	if($companyID != ""){
		...
		if($count > 0){	
			...
			<div class="controlBar">
				<div class="floatLeft">
					<input name="previewInv" type="button" id="previewInv" value="Preview Invoice" class="buttonBlue" onclick="previewInvoice();" />
	...
	function previewInvoice() {
		console.log("previewInvoice function called");
		
		var empty = validateCheckboxes();
		console.log("validateCheckboxes result:", empty);
		
		if(!empty){
			// Get selected trade IDs
			var selectedTrades = [];
			$("input[name='transID[]']:checked").each( function() {
				selectedTrades.push($(this).val());
			});
			console.log("Selected trades:", selectedTrades);
			
			// Get company ID from the URL parameter
			...
			// Open preview window
			var tradeIDs = selectedTrades.join(",");
			var previewURL = "previewInvoice.php?tradeIDs="+tradeIDs+"&companyID="+companyID;
			console.log("Opening preview URL:", previewURL);
			
			// Use window.open instead of newWindow to avoid cookie dependency
			window.open(previewURL, "previewWindow", "width=800,height=900,scrollbars=yes,resizable=yes");
		}
		else
			alert("You must select trades before previewing invoice!");
	}
	</script>
