<?php
			...
			<div class="groupContent">
				<table border="0" cellspacing="5" cellpadding="0">
				 ...
				 <tr>
					...
					<td><select name="companyName" id="companyName" onchange="updateCompanyInfo(this);">
						<?php
							populateExistingCompaniesForSelection();
						?>
					      </select></td>
				 ...
				 <tr>
				   ...
				   <td><select name="parentCo" id="parentCo" onchange="updateParentCompany(this);">
						<?php
	...
	function updateCompanyInfo(selectElement) {
		var selectedCompany = selectElement.value;
		if (selectedCompany) {
			// Update the company name field
			document.getElementById('companyName').value = selectedCompany;
			// Fetch company details to populate invoice fields
			fetchCompanyDetails(selectedCompany);
		}
	}
	
	function updateExistingClient(selectElement) {
		var selectedCompany = selectElement.value;
		if (selectedCompany) {
			// Update the company name field when copying from existing
			document.getElementById('companyName').value = selectedCompany;
			// Fetch company details to populate invoice fields
			fetchCompanyDetails(selectedCompany);
		}
	...
	function updateParentCompany(selectElement) {
		var selectedOption = selectElement.options[selectElement.selectedIndex];
		var companyName = selectedOption.text.split(' (Traders:')[0]; // Remove the trader count part
		
		if (companyName && companyName !== 'SELECT') {
			// Update both Company Name and Copy From Existing dropdowns
			document.getElementById('companyName').value = companyName;
			
			// Find and select the matching option in Copy From Existing dropdown
			var copyFromDropdown = document.getElementById('existing');
			for (var i = 0; i < copyFromDropdown.options.length; i++) {
				if (copyFromDropdown.options[i].text === companyName) {
					copyFromDropdown.selectedIndex = i;
					break;
				}
			}
	...
	function fetchCompanyDetails(companyName) {
		// Use AJAX to fetch company details and populate invoice fields
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4 && xhr.status === 200) {
				try {
					...
					if (response.success) {
						// Populate invoice fields with company data
						document.getElementById('invoiceName').value = response.invoiceName || '';
						document.getElementById('email').value = response.invoiceEmail || '';
						document.getElementById('street1').value = response.street1 || '';
		...
		xhr.send('action=fetchCompanyDetails&companyName=' + encodeURIComponent(companyName));
	}
	</script>