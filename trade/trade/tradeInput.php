...
session_start();
    ...
    <script type="text/javascript">
			// Function to preview trade confirmation as PDF
			function previewTradeConfirm() {
				console.log('Preview button clicked');
				
				// Create a temporary form to submit the data
				var tempForm = document.createElement('form');
				tempForm.method = 'POST';
				tempForm.action = 'previewTradePDF.php';
				tempForm.target = '_blank';
				
				console.log('Form action:', tempForm.action);
				
				// Get all form elements and add them to the temporary form
				...
				console.log('Form submitted and removed');
			}
	</script>
	...
	************************/
	if($_POST["action"] == "submitTrans"){
		...
		$legID = 1;
		if($_POST["clearedTrade"] == "OTC")
			$loopLimit = 2; // OTC trades create 2 legs (buy and sell) for confidentiality
		else if($_POST["clearedTrade"] != "0")
		...
		if($_POST["clearedTrade"] == "OTC" && $i==1)
			$query.= ",\"".$_POST["buyerID"]."\",\"".$_POST["sellerID"]."\""; // OTC buy leg
		else if($_POST["clearedTrade"] == "OTC" && $i==2)
			$query.= ",\"".$_POST["buyerID"]."\",\"".$_POST["sellerID"]."\""; // OTC sell leg
		else if($_POST["clearedTrade"] != "0" && $i==1)
			...
			// Handle different trade types appropriately
			if($_POST["tradeType"] == "9") { // RECs trades
				$query.= ",\"".$_POST["tradeType"]."\",\"100\",\"100\",\"\",\"0\"".
					",\"0\",\"0\",\"".$_POST["buyerPrice"]."\",\"".$_POST["buyerCurr"]."\",\"".$_POST["sellerPrice"]."\"".
					",\"".$_POST["sellerCurr"]."\",\"0\",\"0\",\"0\",\"0\"".
					",\"0\",\"".reverseDate($_POST["startDate"])."\",\"".reverseDate($_POST["endDate"])."\",\"0\"".
					",\"".$_POST["qty"]."\",\"0\",\"".$_POST["totalQty"]."\",\"0\",\"0\"";
			} else { // Standard trade types
			...
			//OTC trade - buy leg shows buyer broker, sell leg shows seller broker
			else if($_POST["clearedTrade"] == "OTC" && $i==1)
				$query.= ",\"".$_POST["buyBroker"]."\",\"".$_POST["buyBrokerRate"]."\",\"".removeSpecialChars($_POST["buyBrokerTotal"])."\",\"\",\"\",\"\"";
			else if($_POST["clearedTrade"] == "OTC" && $i==2)
				$query.= ",\"\",\"\",\"\",\"".$_POST["sellBroker"]."\",\"".$_POST["sellBrokerRate"]."\",\"".removeSpecialChars($_POST["sellBrokerTotal"])."\"";
			...
			if(count(array($_POST["buyBrokerArr"])) > 0 || count(array($_POST["sellBrokerArr"])) > 0) { //multiple brokers added to trade
				...
				//OTC trade - buy leg gets buyer brokers, sell leg gets seller brokers
				else if($_POST["clearedTrade"] == "OTC" && $i==1)
					addBrokers($transRefID,$_POST["buyBrokerArr"], $_POST["buyBrokerRateArr"], $_POST["buyBrokerTotalArr"],1);
				else if($_POST["clearedTrade"] == "OTC" && $i==2)
					addBrokers($transRefID,$_POST["sellBrokerArr"], $_POST["sellBrokerRateArr"], $_POST["sellBrokerTotalArr"],0);
			...
			// Only populate custom fields for the first leg (buy leg) to avoid duplicates
			if($i == 1) {
				if($_POST["tradeType"] == 8) { //crude oil custom fields
					populateCustomFields($_POST,$transRefID);
				}
				else if($_POST["tradeType"] == 9) { //RECs custom fields
					populateCustomFields($_POST,$transRefID);
				}
			}
		}
	...
	*****************************/
		...
		while(list($updateID,$transactionID,$leg) = mysqli_fetch_row($resultTrades)){
			...
			// Handle different trade types appropriately
			if($_POST["tradeType"] == "9") { // RECs trades
				$query.= ", fkProductGroup=\"".$_POST["tradeType"]."\", fkProductID=\"100\", fkProductTypeID=\"100\"".
					", fldRegion=\"\", fldLocation=\"0\", fldOffsetIndex=\"0\", fldDelivery=\"0\"".
					...
					", fkOptionStyleID=\"0\", fkOptionTypeID=\"0\", fkExerciseType=\"0\", fldStrike=\"0\"".
					", fldStrikeCurrency=\"0\", fldStartDate=\"".reverseDate($_POST["startDate"])."\", fldEndDate=\"".reverseDate($_POST["endDate"])."\"".
					", fldBlock=\"0\", fldUnitQty=\"".$_POST["qty"]."\", fldUnitFreq=\"0\", fldTotalQuantity=\"".$_POST["totalQty"]."\", fldHours=\"0\", fldTimeZone=\"0\"";
			} else { // Standard trade types
	...
	*************************/
		...
		for($i=$legID; $i<=(($legID+$loopLimit)-1); $i++){
			...
			if($_POST["clearedTrade"] != "0" && ($loopCount+1)==1)
				...
				$query.= ",\"".$_POST["buyerID"]."\",\"".$_POST["sellerID"]."\"";
			// Handle RECs trades differently - set non-applicable fields to defaults
			if($_POST["tradeType"] == "9") { // RECs trade
				$query.= ",\"".$_POST["tradeType"]."\",\"0\",\"0\"".
					",\"".$_POST["region"]."\",\"".$_POST["location"]."\",\"0\",\"".$_POST["delivery"]."\"".
					",\"".$_POST["buyerPrice"]."\",\"".$_POST["buyerCurr"]."\",\"".$_POST["sellerPrice"]."\",\"".$_POST["sellerCurr"]."\",\"0\"".
					",\"0\",\"0\",\"0\",\"0\"".
			...
			$query.=",\"".$_SESSION["CONST_PENDING"]."\""; //status default to pending (0)
	...
	if($step == "confirm") {
		...
		//$tradeType = $tradeType;
			...
			if($result2) {
				...
				echo("Error: Template not found for trade type " . $_GET["tradeType"] . "<br>");
				$templateOptions = array_fill(0, 19, 0); // Create array with default values
			}
			/*
			0 = pkTemplateID
			1 = fldTemplateName
			...
			*/			
			...
			if($_GET["tradeID"] != ""){
				...
				if($count == 1){
					...
					// Get the REC and New Product 2 trade type IDs dynamically
					$customTradeQuery = "SELECT pkTypeID FROM tblB2TTradeType WHERE fldTradeName IN ('REC', 'New Product 2')";
					$customTradeResult = mysqli_query($_SESSION['db'], $customTradeQuery);
					$customTradeTypeIDs = array();
					if($customTradeResult && mysqli_num_rows($customTradeResult) > 0) {
						while($row = mysqli_fetch_row($customTradeResult)) {
							$customTradeTypeIDs[] = $row[0];
						}
					}
				  ...
				  <tr>
					...
					<td class="yui-skin-sam">
						...
						<img src="../images/calendar.jpg" height="15" width="18" alt="Cal" id="show1up" class="center" />
					  	...
					  	<a href="#" class="link-close" onclick="YAHOO.example.calendar.cal1.hide(); return false;">
						...
						<script language="javascript" type="text/javascript">
							...
							YAHOO.example.calendar.init = function() { 
								...
								// Ensure calendar starts hidden
								YAHOO.example.calendar.cal1.hide();
								 
								// Listener to show the single page Calendar when the button is clicked 
								YAHOO.util.Event.addListener("show1up", "click", function() {
									// Position calendar near the button before showing
									var button = document.getElementById("show1up");
									var container = document.getElementById("cal1Container");
									var buttonRect = button.getBoundingClientRect();
									
									// Position calendar below and to the right of the button
		    ...
		    <table border="0" cellspacing="3" cellpadding="0">
				  <tr>
					<td colspan="2" valign="top"><table border="0" cellspacing="3" cellpadding="0">
                      ...
                      <?php
                      // Only show Product and Product Type dropdowns for non-RECs trades
                      if($_GET["tradeType"] != "9") {
                      ?>
                      <tr>
                      ...
                      <?php
					  //hide for Crude Oil trades only (New Product trades should show Location/Region)
					  $hideStandardLocationFields = false;
					  
					  // Check if this is a REC trade type or New Product 2 trade type
					  $customTradeQuery = "SELECT pkTypeID FROM tblB2TTradeType WHERE fldTradeName IN ('REC', 'New Product 2') OR pkTypeID = 9";
					  $customTradeResult = mysqli_query($_SESSION['db'], $customTradeQuery);
					  $customTradeTypeIDs = array();
					  if($customTradeResult && mysqli_num_rows($customTradeResult) > 0) {
						  while($row = mysqli_fetch_row($customTradeResult)) {
					  ...
					  // Hide if Crude Oil trade only (not New Product trades or RECs trades)
					  if($templateOptions[18] == "1" && $_GET["tradeType"] != "9") {
						  $hideStandardLocationFields = true;
					  }
					  
					  if(!$hideStandardLocationFields){
					  ?>
                      <?php
                      // Only show Location dropdown for non-RECs trades
                      if($_GET["tradeType"] != "9") {
                      ?>
					  <tr>
                      ...
                      ?>
                      <?php
                      // Only show Region field for non-RECs trades
                      if($_GET["tradeType"] != "9") {
                      ?>
					  <tr>
					  ...
					  <?php
					  // Show Offset Index for trades that have template option enabled OR for REC/New Product trades
					  // But hide it for RECs trades (type 9) since it's not applicable
					  if(($templateOptions[2] == "1" || in_array($_GET["tradeType"], $customTradeTypeIDs)) && $_GET["tradeType"] != "9"){
					  ?>
					  <tr>
					    ...
					    <td style="height: 20px; width: 170px;">
							<select name="offsetIndex" id="offsetIndex" style="width: 170px;" onfocus="toggleWidth(this);" onblur="resetWidth(this);">
					     	<?php
								populateOffsetList($offsetIndex, $_GET["tradeType"]);
							?>
						 	</select>
                      ...
                      <?php
					  //custom fields for Crude Oil trades OR RECs trades
					  if($templateOptions[18] == "1" || $_GET["tradeType"] == "9"){
						  ...
						  if(isset($_GET["tradeID"])) {
							  ...
							  // First get the transaction number for this leg
							  $queryTransNum = "SELECT fldTransactionNum FROM tblB2TTransaction WHERE pkTransactionID=".$_GET["tradeID"];
							  $resultTransNum = mysqli_query($_SESSION['db'],$queryTransNum);
							  if($resultTransNum && mysqli_num_rows($resultTransNum) > 0) {
								  list($transactionNum) = mysqli_fetch_row($resultTransNum);
								  
								  // Now find custom fields from any leg of the same transaction
								  $queryCustom = "SELECT cf.* FROM tblB2TCustomFields cf 
												  WHERE cf.fkTransID IN (
												      SELECT pkTransactionID FROM tblB2TTransaction 
												      WHERE fldTransactionNum = '$transactionNum'
												  ) 
								  LIMIT 1";
								  
								  $resultCustom = mysqli_query($_SESSION['db'],$queryCustom);
								  if($resultCustom && mysqli_num_rows($resultCustom) > 0) {
									  list($recordID,$productCustom,$priceCustom,$deliveryPoint,$additionalInfo,$fkTransID) = mysqli_fetch_row($resultCustom);
								  } else {
									  // Initialize empty values if no custom fields found
									  $productCustom = $priceCustom = $deliveryPoint = $additionalInfo = "";
								  }
							  } else {
								  // Initialize empty values if transaction not found
								  $productCustom = $priceCustom = $deliveryPoint = $additionalInfo = "";
							  }
						  } else {
							  // For new trades, initialize custom field variables
							  $productCustom = $priceCustom = $deliveryPoint = $additionalInfo = "";
						  }
					  ?>
				      
				      <?php
				      // Show custom fields for RECs trades
				      if($_GET["tradeType"] == "9"){
				      ?>
				      <tr>
                        <td nowrap="nowrap" class="fieldLabel">Product: </td>
                      ...
                      <?php
					  // For New Product trades, we use the existing standard fields
					  // The only difference is the MMBTU/Block selector in the Unit Quantity field
					  // which is already handled in the existing code above
					  
					  // Get the REC and New Product 2 trade type IDs for buyer field visibility
					  $customTradeQuery = "SELECT pkTypeID FROM tblB2TTradeType WHERE fldTradeName IN ('REC', 'New Product 2')";
					  $customTradeResult = mysqli_query($_SESSION['db'], $customTradeQuery);
					  $customTradeTypeIDs = array();
					  if($customTradeResult && mysqli_num_rows($customTradeResult) > 0) {
				    ...
				    <td colspan="2" valign="top"><table border="0" cellspacing="3" cellpadding="0">
				     ...
				     <tr>
					  ...
					  ?>
					  					  ...
					  					  <?php
					  // Show Buyer Price for REC/New Product trades OR when template allows it
					  $showBuyerPrice = false;
					  if($templateOptions[4] == "1" || in_array($_GET["tradeType"], $customTradeTypeIDs)) {
						  $showBuyerPrice = true;
					  }
				     ...
				     <tr>
					  ...
					  <?php
					  // Show Buyer Currency for REC/New Product trades OR when template allows it
					  $showBuyerCurrency = false;
					  if($templateOptions[5] == "1" || in_array($_GET["tradeType"], $customTradeTypeIDs)) {
						  $showBuyerCurrency = true;
					  }
					  
					  if($showBuyerCurrency){
					  ?>
				      <tr>
				    ...
				    <td valign="top"><table border="0" cellspacing="3" cellpadding="0">
				      <tr>
				        ...
				        <td class="yui-skin-sam"><input name="startDate" type="text" id="startDate" size="10" value="<?php echo(formatDate($startDate)); ?>" onchange="updateQuantity();" />
							...
							<div id="cal2Container" class="adjust">
								<a href="#" class="link-close" onclick="YAHOO.example.calendar2.cal2.hide(); return false;">
									<span class="close-icon calclose">Close</span>
								</a>
							</div>
							...
							<script language="javascript" type="text/javascript">
								...
								function mySelectHandler2(type,args,obj) {
									...
									// Ensure calendar starts hidden
									YAHOO.example.calendar2.cal2.hide();
								 
									// Listener to show the single page Calendar when the button is clicked 
									YAHOO.util.Event.addListener("show2up", "click", function() {
										// Position calendar near the button before showing
										var button = document.getElementById("show2up");
										...
										YAHOO.example.calendar2.cal2.show();
									}, YAHOO.example.calendar2.cal2, true);
								} 
				      ...
				      <tr>
				        ...
				        <td class="yui-skin-sam"><input name="endDate" type="text" id="endDate" size="10" value="<?php echo(formatDate($endDate)); ?>" onchange="updateQuantity();" />
							...
							<div id="cal3Container">
								<a href="#" class="link-close" onclick="YAHOO.example.calendar3.cal3.hide(); return false;">
									<span class="close-icon calclose">Close</span>
								</a>
							</div>
							...
							<script language="javascript" type="text/javascript">
								...
								YAHOO.example.calendar3.init = function() { 
									YAHOO.example.calendar3.cal3 = new YAHOO.widget.Calendar("cal3","cal3Container", { 
										title:"Choose a date:", 
										close:true
									});
									...
									// Ensure calendar starts hidden
									YAHOO.example.calendar3.cal3.hide();
								 
									// Listener to show the single page Calendar when the button is clicked 
									YAHOO.util.Event.addListener("show3up", "click", function() {
										// Position calendar near the button before showing
										var button = document.getElementById("show3up");
										var container = document.getElementById("cal3Container");
										var buttonRect = button.getBoundingClientRect();
										...
										YAHOO.example.calendar3.cal3.show();
									}, YAHOO.example.calendar3.cal3, true);
								}
				      ...
				      <tr>
					   ...
					   <?php
					  // Show Unit Frequency for trades that have template option enabled OR for REC/New Product trades
					  if($templateOptions[16] == "1" || in_array($_GET["tradeType"], $customTradeTypeIDs)){
					  ?>
				      <tr>
		...
		<div class="fieldGroup groupHalf" style="margin-right: 15px;">
			...
			<div class="groupContent">
               ...
               <br />
				  ...
				  <tr>
					...
					<td><select name="buyBroker" id="buyBroker">
					   <?php
						...
						// For new trades, default to logged-in user's broker ID
						else if(empty($buyBroker) && $_GET["tradeID"] == "") {
							$buyBroker = $_SESSION["brokerID42"];
						}
                ...
                <div id="moreBuyBrokers"><?php if($_GET['tradeID'] != "") { getBrokers("buy",$transactionID,$showClearID,$tradeID,$status); } ?></div>
		...
		<div class="fieldGroup groupHalf">
		  ...
		  <div class="groupContent">
             ...
             <br />
				  ...
				  <tr>
				    ...
				    <td><select name="sellBroker" id="sellBroker">
                         <?php
							...
							// For new trades, default to logged-in user's broker ID
							else if(empty($sellBroker) && $_GET["tradeID"] == "") {
								$sellBroker = $_SESSION["brokerID42"];
							}
                ...
                <div id="moreSellBrokers"><?php if($_GET['tradeID'] != "") { getBrokers("sell",$transactionID,$showClearID,$tradeID,$status); } ?></div>
		...
		<div>
			...
			<div class="floatCenter" style="text-align: center;">
				<?php
				// Show Preview Confirm button only for new trades
				if($_GET["tradeID"] == "" && $status != 3){
					?>
					<input type="button" name="previewConfirm" value="Preview Confirm" onclick="previewTradeConfirm();" class="buttonBlue" />
					<?php
				}
				?>
			</div>
		...
		<script language="javascript" type="text/javascript">
			...
			// Function removed - no longer needed since we use standard fields
			
			// Function to toggle Pricing Model textarea
			function togglePricingModel() {
				var checkbox = document.getElementById('pricingModelEnabled');
				var textarea = document.getElementById('pricingModel');
			...
			// Function to toggle Settlement Terms textarea
			function toggleSettlementTerms() {
				var checkbox = document.getElementById('settlementTermsEnabled');
				var textarea = document.getElementById('settlementTerms');
