	...
	***********************/
	public function init($buyerName,$sellerName) {
		...
		if(isset($this->region) && !empty($this->region)) {
			$conditions[] = "fldRegion=\"".$this->region."\"";
		}
		if(isset($this->productType) && !empty($this->productType)) {
			$conditions[] = "fkProductTypeID=\"".$this->productType."\"";
		}
		if(isset($this->locationID) && !empty($this->locationID)) {
			$conditions[] = "fkLocationID=\"".$this->locationID."\"";
		}
		
		// If no conditions, use a default condition to avoid invalid SQL
		if (empty($conditions)) {
			$conditions[] = "1=1";
		}
		...
		if (!$result) {
			error_log("ConfirmObj query failed: " . mysqli_error($_SESSION['db']) . " Query: " . $query);
			$count = 0;
			$companyName = $showProduct = $showRegion = $showBuyerCurrency = $showSellerPrice = $sellerPriceText = $productTypeText = $showDelivery = null;
		} else {
